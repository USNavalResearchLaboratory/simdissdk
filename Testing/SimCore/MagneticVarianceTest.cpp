...
namespace
{
  int calculateMagneticVarianceTest()
  {
    ...
    /////////////////////////////////
    // Automatically generated using values from: ./WMM2025_TEST_VALUES.txt
    // Geodetic angle values in degrees, convert to rad
    // Height above WGS84 ellipsoid values in km, convert to m
    // Declination (variance) values are in deg)

    lla.set(80.0 * simCore::DEG2RAD, 0.0 * simCore::DEG2RAD, 0.0*1000.);
    funcRv = wmm.calculateMagneticVariance(lla, 1, 2025, varianceRad);
    rv += SDK_ASSERT(simCore::areAnglesEqual(varianceRad * simCore::RAD2DEG, 1.28, 0.01) && funcRv == 0);

    lla.set(0.0 * simCore::DEG2RAD, 120.0 * simCore::DEG2RAD, 0.0*1000.);
    funcRv = wmm.calculateMagneticVariance(lla, 1, 2025, varianceRad);
    rv += SDK_ASSERT(simCore::areAnglesEqual(varianceRad * simCore::RAD2DEG, -0.16, 0.01) && funcRv == 0);

    lla.set(-80.0 * simCore::DEG2RAD, 240.0 * simCore::DEG2RAD, 0.0*1000.);
    funcRv = wmm.calculateMagneticVariance(lla, 1, 2025, varianceRad);
    rv += SDK_ASSERT(simCore::areAnglesEqual(varianceRad * simCore::RAD2DEG, 68.78, 0.01) && funcRv == 0);

    lla.set(80.0 * simCore::DEG2RAD, 0.0 * simCore::DEG2RAD, 100.0*1000.);
    funcRv = wmm.calculateMagneticVariance(lla, 1, 2025, varianceRad);
    rv += SDK_ASSERT(simCore::areAnglesEqual(varianceRad * simCore::RAD2DEG, 0.85, 0.01) && funcRv == 0);

    lla.set(0.0 * simCore::DEG2RAD, 120.0 * simCore::DEG2RAD, 100.0*1000.);
    funcRv = wmm.calculateMagneticVariance(lla, 1, 2025, varianceRad);
    rv += SDK_ASSERT(simCore::areAnglesEqual(varianceRad * simCore::RAD2DEG, -0.15, 0.01) && funcRv == 0);

    lla.set(-80.0 * simCore::DEG2RAD, 240.0 * simCore::DEG2RAD, 100.0*1000.);
    funcRv = wmm.calculateMagneticVariance(lla, 1, 2025, varianceRad);
    rv += SDK_ASSERT(simCore::areAnglesEqual(varianceRad * simCore::RAD2DEG, 68.21, 0.01) && funcRv == 0);

    lla.set(80.0 * simCore::DEG2RAD, 0.0 * simCore::DEG2RAD, 0.0*1000.);
    funcRv = wmm.calculateMagneticVariance(lla, 183, 2027, varianceRad);
    rv += SDK_ASSERT(simCore::areAnglesEqual(varianceRad * simCore::RAD2DEG, 2.59, 0.01) && funcRv == 0);

    lla.set(0.0 * simCore::DEG2RAD, 120.0 * simCore::DEG2RAD, 0.0*1000.);
    funcRv = wmm.calculateMagneticVariance(lla, 183, 2027, varianceRad);
    rv += SDK_ASSERT(simCore::areAnglesEqual(varianceRad * simCore::RAD2DEG, -0.24, 0.01) && funcRv == 0);

    lla.set(-80.0 * simCore::DEG2RAD, 240.0 * simCore::DEG2RAD, 0.0*1000.);
    funcRv = wmm.calculateMagneticVariance(lla, 183, 2027, varianceRad);
    rv += SDK_ASSERT(simCore::areAnglesEqual(varianceRad * simCore::RAD2DEG, 68.49, 0.01) && funcRv == 0);

    lla.set(80.0 * simCore::DEG2RAD, 0.0 * simCore::DEG2RAD, 100.0*1000.);
    funcRv = wmm.calculateMagneticVariance(lla, 183, 2027, varianceRad);
    rv += SDK_ASSERT(simCore::areAnglesEqual(varianceRad * simCore::RAD2DEG, 2.16, 0.01) && funcRv == 0);

    lla.set(0.0 * simCore::DEG2RAD, 120.0 * simCore::DEG2RAD, 100.0*1000.);
    funcRv = wmm.calculateMagneticVariance(lla, 183, 2027, varianceRad);
    rv += SDK_ASSERT(simCore::areAnglesEqual(varianceRad * simCore::RAD2DEG, -0.23, 0.01) && funcRv == 0);

    lla.set(-80.0 * simCore::DEG2RAD, 240.0 * simCore::DEG2RAD, 100.0*1000.);
    funcRv = wmm.calculateMagneticVariance(lla, 183, 2027, varianceRad);
    rv += SDK_ASSERT(simCore::areAnglesEqual(varianceRad * simCore::RAD2DEG, 67.93, 0.01) && funcRv == 0);
