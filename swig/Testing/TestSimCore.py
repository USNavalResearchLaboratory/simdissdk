...
ray.direction.set(0, 0, -1)
distance_in_meters = simCore.rayIntersectsSphere(ray, s)
assert(distance_in_meters is not None)
assert(distance_in_meters == 9.0)
ray.direction.set(1, 0, 0)
distance_in_meters = simCore.rayIntersectsSphere(ray, s)
assert(distance_in_meters is None)
e = simCore.Ellipsoid()
ray.origin.set(0, 0, 10.)
ray.direction.set(0, 0, -1)
distance_in_meters = simCore.rayIntersectsEllipsoid(ray, e)
assert(distance_in_meters is not None)
assert(distance_in_meters == 9.0)
ray.direction.set(1, 0, 0)
distance_in_meters = simCore.rayIntersectsEllipsoid(ray, e)
...
success, velOut, oriOut = simCore.calculateVelOriFromPos(v1, v2, 1, simCore.COORD_SYS_LLA, v3)
assert(success is not None and velOut is not None and oriOut is not None)
success, velOut, oriOut = simCore.calculateVelOriFromPos(v1, v2, 1, simCore.COORD_SYS_ECEF, v3, simCore.COORD_SYS_XEAST)
assert(success is not None and velOut is not None and oriOut is not None)
