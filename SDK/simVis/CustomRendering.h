...
public:
  ...
  */
  ...
  /*
   * Abstract class that can be implemented to determine which ECEF locations to
   * use when checking if this CustomRenderingNode is being picked by the mouse
   */
  ...
  /// Sets the AbstractPointPicker implementation to use
  void setPointPicker(const std::shared_ptr<AbstractPointPicker>& pointPicker);

  /** Retrieves the currently visible points representing the custom */
  void getPickingPoints(std::vector<osg::Vec3d>& ecefVec) const;
