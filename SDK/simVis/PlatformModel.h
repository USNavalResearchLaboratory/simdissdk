...
namespace simVis
{
  ...
  */
  ...
  private:
    ...
    /// Updates the scale based on pref or if force is set to true when the model has changed; returns true if changed
    bool updateScale_(const simData::PlatformPrefs& prefs, bool force);
    /// Updates the XYZ scale based on pref or if force is set to true when the model has changed; returns true when scale changes
    bool updateScaleXyz_(const simData::PlatformPrefs& prefs, bool force);
    /// Updates the dynamic scale based on pref or if force is set to true when the model has changed; returns true when dynamic scale changes
