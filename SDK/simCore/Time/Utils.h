...
namespace simCore
{
  ...
  */
  ...
  /**
   * Determines a "go to" time based on user input of hours, minutes, and seconds, closest to the current time,
   * bounded by scenario begin/end. This is useful for processing nearly free-form input for time modification
   * routines, allowing the user easily jump to a nearby time by only changing seconds, minutes, and/or hours.
   * If request only has seconds provided, seconds will be applied to the current time's existing minutes.
   * If request only has minutes and seconds provided, minutes and seconds will be applied to the current time's
   * existing hours.
   * If request provides hours, nearest time (possibly resulting in day rollover or day rollback) will be calculated.
   * In the hours case, for a tie on nearest time, same day is preferred.
   * @param[in] currentTime  Current time
   * @param[in] startTime  Start time of the time bounds
   * @param[in] endTime  End time of the time bounds
   * @param[in] request  Requested go to time, usually generated by simCore::parseFreeFormTimeStr()
   * @return  Relevant go to time that's nearest to the current time
   */
  SDKCORE_EXPORT std::optional<simCore::TimeStamp> getNearestGoToTime(const simCore::TimeStamp& currentTime,
    const simCore::TimeStamp& startTime, const simCore::TimeStamp& endTime, const simCore::FreeFormResult& request);
