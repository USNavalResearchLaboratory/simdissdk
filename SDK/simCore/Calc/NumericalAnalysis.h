...
/**
 * Computes an Exponentially Weighted Moving Average step. This is useful for calculating, for example,
 * bytes per second or messages per second, over a period of time.
 * @param instantaneousValue The newly measured instantaneous value (e.g., current Hz).
 * @param previousAverage The historical average from the last step.
 * @param alpha The weight of the new sample [0.0 = all history, 1.0 = all new sample].
 * @param snapToZeroThreshold If the result falls below this, it snaps to 0.0 to prevent micro-decay tail.
 * @return The updated moving average.
 */
SDKCORE_EXPORT double calculateEwma(double instantaneousValue, double previousAverage, double alpha = 0.6,
  double snapToZeroThreshold = 0.05);

/**
 * General-purpose tracker for asynchronous or bursty events.
 * Accumulates events silently and calculates an EWMA rate lazily upon querying.
 * Useful as a data rate calculator, for message counts, bandwidth, or other
 * frequently-changing values.
 */
class SDKCORE_EXPORT EventRateTracker
{
public:
  /**
   * Instantiates the rate tracker with given values.
   * @param alpha The weight of the new sample [0.0 = all history, 1.0 = all new sample].
   * @param snapThreshold If the result falls below this, it snaps to 0.0 to prevent micro-decay tail.
   */
  explicit EventRateTracker(double alpha = 0.6, double snapThreshold = 0.05);

  /**
   * Call this whenever an event occurs (e.g., 1 message, or 500 bytes).
   * Highly efficient: zero clock lookups, zero floating-point math.
   */
  void add(uint64_t count = 1);

  /**
   * Calculates and returns the smoothed rate (per second) based on
   * accumulated events since the last evaluation. Because this recalculates
   * the smoothed rate, this is not a const function. Call this periodically,
   * e.g. once per UI tick or reporting cycle.
   * @return Newly computed rate per second.
   */
  double evaluate();

  /**
   * Retrieves the current smoothed rate without updating accumulators or recalculating.
   * This is useful for getting const access to the rate that gets calculated on a
   * more regular timer, e.g. for use in a QAbstractItemModel::data() call.
   * @return Most recently computed rate per second.
   */
  double getRate() const;

  /// Resets accumulator and rate history
  void clear();

private:
  double alpha_ = 0.6;
  double snapThreshold_ = 0.05;
