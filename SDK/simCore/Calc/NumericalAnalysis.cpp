...
double calculateEwma(double instantaneousValue, double previousAverage, double alpha, double snapToZeroThreshold)
{
  const double result = (alpha * instantaneousValue) + ((1.0 - alpha) * previousAverage);
  return (result < snapToZeroThreshold) ? 0.0 : result;
}

////////////////////////////////////////////////////////////////////////////

EventRateTracker::EventRateTracker(double alpha, double snapThreshold)
  : alpha_(alpha),
...
void EventRateTracker::add(uint64_t count)
{
  accumulator_ += count;
}

double EventRateTracker::evaluate()
{
  const auto now = std::chrono::steady_clock::now();
  const std::chrono::duration<double> elapsed = now - lastSampleTime_;
  const double dt = elapsed.count();

  // Only recompute if enough time has passed (e.g., > 10ms) to avoid divide-by-zero
  // on rapid consecutive getRate() calls.
  if (dt > 0.010)
  {
    const double instRate = static_cast<double>(accumulator_) / dt;
    currentRate_ = calculateEwma(instRate, currentRate_, alpha_, snapThreshold_);
...
double EventRateTracker::getRate() const
{
  return currentRate_;
}

void EventRateTracker::clear()
{
  accumulator_ = 0;
  currentRate_ = 0.0;
  lastSampleTime_ = std::chrono::steady_clock::now();
