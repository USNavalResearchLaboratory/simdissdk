...
namespace simCore
{
  ...
  * @param[out] tpVec Distance in XYZ that the sphereVec is from the llaVec location
  ...
  */
  SDKCORE_EXPORT void sphere2TangentPlane(const Vec3& llaVec, const Vec3& sphereVec, Vec3& tpVec, const Vec3* sphereTpOrigin=NULL);
  ...
  * sphere Earth XYZ values are available, they can be given for a faster calculation.
  * @param[in ] llaVec Latitude, longitude, and altitude values of the tangent plane origin
  * @param[in ] tpVec Given distance in XYZ that the sphereVec is from the llaVec location
  * @param[out] sphereVec XYZ values to convert to an ENU location
  * @param[in ] sphereTpOrigin Perfect sphere coordinates of the tangent plane origin, if known.  If this value is not known, it will be calculated from llaVec.  Calculation is faster if this is provided.
  */
  SDKCORE_EXPORT void tangentPlane2Sphere(const Vec3 &llaVec, const Vec3 &tpVec, Vec3& sphereVec, const Vec3* sphereTpOrigin=NULL);
  ...
  * @param[out] azfwd Forward azimuth from reference to second point clockwise from North (rad), not calculated if NULL
  * @param[out] azbck Backward azimuth from second point to reference (rad), not calculated if NULL
  * @return dist, Geodesic length or distance from reference to second point (m)
  */
  SDKCORE_EXPORT double sodanoInverse(const double refLat, const double refLon, const double refAlt, const double lat, const double lon, double *azfwd=NULL, double *azbck=NULL);
  ...
  */
   ...
   * @param[out] wrapsDateline Set to true if non-null and if the area wraps the antimeridian (dateline).  NULL is acceptable.
   */
